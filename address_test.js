  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!campaignDataCache.has(Data.address)) {
          // Fetch story using axios
          const storyResponse = await axios.get(Data.storyUrl);
          ...
          const cachedData = {
            story: storyData,
            myDonations: MyAllDonations.map((e) => {
              return {
                donar: e.args.donar,
          ...
          campaignDataCache.set(Data.address, cachedData);
        } else {
          const cachedData = campaignDataCache.get(Data.address);
          storyData = cachedData.story;
          setMyDonations(cachedData.myDonations);
        }
